...
public class UploadUtil {
	...
	private static int readTimeOut = 3600 * 1000; // 读取超时
	private static int connectTimeout = 3600 * 1000; // 超时时间
	...
	public static String uploadFile(MultipartFile file, String fileKey, String RequestURL, Map<String, String> param) {
		...
		try {
			...
			if (res == 200) {
				/*
				 * InputStream input = conn.getInputStream(); StringBuffer sb1 =
				 * new StringBuffer(); int ss; while ((ss = input.read()) != -1)
				 * { sb1.append((char) ss); } result = sb1.toString();
				 */
